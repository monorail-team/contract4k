...
fun main() {
    ...
    // 1) 올바른 데이터로 테스트
    val goodOrder = Order(amount = 3000, status = OrderStatus.CREATED)
    val goodCustomer = Customer(name = "Alice", age = 30)
    try {
    ...
    // 2) 잘못된 데이터로 테스트
    val badOrder = Order(amount = -10, status = OrderStatus.CREATED)
    val badCustomer = Customer(name = "", age = 20)
    try {
